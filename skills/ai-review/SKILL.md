...
5. Fetch and store Qodo inline root comments and its latest issue summary. Process Qodo after CodeRabbit; reply that an overlapping finding was already addressed with its commit SHA. Qodo threads cannot be resolved programmatically, so an inline reply is the resolution signal.
